...
- **Electoral Geography**: Complete Nigerian electoral structure seeded: 109 senatorial districts, 348 federal constituencies across 37 states. LGAs mapped to senatorial districts (772/774, 99.7%) and federal constituencies (767/774, 99.1%). Tables: `senatorial_districts`, `federal_constituencies`. LGAs have `senatorial_district_id` and `federal_constituency_id` foreign keys.
- **General Election Management**: Admin page at `/admin/general-elections` with bulk election creation (creates elections for selected positions across all states), bulk status updates, filtering by position/status/state/year, and deletion. Supports all election classes: Presidential, Governorship, Senatorial (district-level), House of Reps (constituency-level), State Assembly, LGA Chairman, Councillorship.
- **Geographic Election Filtering**: Both agent login and user-facing elections use precise geographic filtering: Presidential=all, Governorship/State Assembly=state, Senatorial=senatorial district (fallback to state), House of Reps=federal constituency (fallback to state), LGA Chairman=LGA, Councillorship=ward. User "My Area" toggle on general elections page filters elections by logged-in user's location.
